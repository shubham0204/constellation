...
actual val targetModule: Module = module {
    single<FileUtils> { FileUtils(applicationContext = get()) }
    single<AppDatabase> { DBProvider(context = get()).getDatabase() }
    single<Model2Vec> { Model2Vec(fileUtils = get()) }
    single<SimilarThoughtsService> { SimilarThoughtsService(model2vec = get()) }
    single<SimilarThoughtsService> { SimilarThoughtsService(model2vec = get()) }
    single<SentimentAnalyzer> { SentimentAnalyzer(fileUtils = get()) }
